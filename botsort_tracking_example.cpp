...
/**
 * @brief Check if the path points to a YOLO model file (.xml or .onnx)
 */
bool is_yolo_model_path(const std::string &path)
{
    size_t dot = path.find_last_of('.');
    if (dot == std::string::npos)
        return false;
    std::string ext = path.substr(dot + 1);
    return (ext == "xml" || ext == "onnx");
}
...
int main(int argc, char **argv)
{
    ...
    if (argc < 4)
    {
        std::cout << "Usage eg. 1 (YOLO live detection): "
                     "./botsort_tracking_example "
                     "<source> <yolo_model_path(.xml|.onnx)> "
                     "<dir_to_save_mot_format_output>"
                  << std::endl;
        std::cout << "Usage eg. 2 (file-based): ./botsort_tracking_example "
                     "<source> "
                     "<dir_containing_per_frame_detections> "
                     "<dir_to_save_mot_format_output>"
                  << std::endl;
        std::cout
                << "Usage eg. 3 (YOLO + full config): "
                   "./botsort_tracking_example "
                   "<tracker_config_path> <gmc_config_path> <reid_config_path> "
                   "<reid_onnx_model_path> "
                   "<source> <yolo_model_path> <dir_to_save_mot_format_output>"
                << std::endl;
        std::cout
                << "Usage eg. 4 (file-based + full config): "
                   "./botsort_tracking_example "
                   "<tracker_config_path> <gmc_config_path> <reid_config_path> "
                   "<reid_onnx_model_path> "
    ...
    {
        std::cout << "Invalid number of arguments. See usage above."
                  << std::endl;
        return -1;
    }
    ...
    std::string tracker_config_path, gmc_config_path, reid_config_path,
            reid_onnx_model_path, source, labels_or_model_path, output_dir,
            gt_filepath;
    bool use_yolo = false;
    ...
    {
        source = argv[1];
        labels_or_model_path = argv[2];
        output_dir = argv[3];
        use_yolo = is_yolo_model_path(labels_or_model_path);
    }
    else if (argc == 8)
    {
        ...
        source = argv[5];
        labels_or_model_path = argv[6];
        output_dir = argv[7];
        use_yolo = true;
        if (!is_yolo_model_path(labels_or_model_path))
        {
            std::cerr << "Error: Expected YOLO model path (.xml or .onnx) for "
                         "8-arg mode."
                      << std::endl;
            return -1;
        }
...
// // Initialize GlobalMotionCompensation
    ...
    */
    ...
    // Initialize YOLO detector when in YOLO mode
#ifdef HAVE_OPENVINO
    ...
    if (use_yolo)
    {
        ...
        detector = std::make_unique<YOLO>(labels_or_model_path, conf_thresh,
                                          nms_thresh, device);
        std::cout << "YOLO detector initialized: " << labels_or_model_path
                  << std::endl;
    }
    ...
    if (use_yolo)
    {
        ...
        while (cap.read(frame) || frame_counter < image_filepaths.size())
        {
            ...
            std::string output_file_img =
                    output_dir_img + "/" + filename + ".jpg";

            // YOLO detection + tracker
            detector->RunInference(frame, detections);
...
#else
        std::cerr << "Build without OpenVINO. Enable BUILD_WITH_YOLO and "
                     "install OpenVINO."
                  << std::endl;
        return -1;
#endif
    ...
    {
        // Read detections from file and execute MultiObjectTracker
        std::string labels_dir = labels_or_model_path;
        while (cap.read(frame) || frame_counter < image_filepaths.size())
        {
            std::string filename;
    ...
    {
        std::string gt_path =
                gt_filepath.empty() ? labels_or_model_path : gt_filepath;
        std::vector<std::vector<Detection>> gt_per_frame =
                read_mot_gt_from_file(gt_path);
    ...
    {
        std::cerr << "For file-based mode, set YOLOv8_PREDS=1 or GT_AS_PREDS=1"
                  << std::endl;
        return -1;
    }
#endif

    if (frame_counter > 0 && tracker_time_total > 0)
    {
        std::cout << "Average tracker FPS: "
                  << frame_counter / tracker_time_total << std::endl;
